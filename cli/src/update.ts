...
const COLLECTION_FEEPOINTS = 1230;
...
programCommand("update").action(async (directory, cmd) => {
  ...
  await program.methods
    .updateMachine(ARWEAVE_MANIFEST_URL, null, null, COLLECTION_FEEPOINTS)
    .accounts({
      machine: machine,
  ...
  console.log(await program.account.feebleMachine.fetchNullable(machine));
});
